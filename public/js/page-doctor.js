...
async function openModal(doctorData) {
  ...
  KEYWORDS.DOCTOR_STATUS.forEach((ele) => {
    ...
    data?.list.forEach((ele) => {
      const rowHtml = `
         <tr class="text-center">
            ...
            <td><p>${ele.note}</p></td>
            <td><b class="txt-status-${String(ele.status).toLocaleLowerCase()}">${KEYWORDS.DOCTOR_STATUS.find((fi) => fi.value == ele.status)?.text ?? ''}</b></td>
