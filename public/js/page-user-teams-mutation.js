...
async function updateTeamStatus(status) {
  const confirmed = window.confirm(`Bạn có chắc chắn muốn ${status === VARIABLE_ENUM.TEAM_STATUS.APPROVED.VALUE ? VARIABLE_ENUM.TEAM_STATUS.APPROVED.TEXT : VARIABLE_ENUM.TEAM_STATUS.REFUSE.TEXT} đội này không?`);
  if (!confirmed) return;

  // Disable buttons
  const approveBtn = document.querySelector(`button[onclick="updateTeamStatus('${VARIABLE_ENUM.TEAM_STATUS.APPROVED.VALUE}')"]`);
