...
async function openModal(homeData) {
  ...
  VARIABLE_ENUM.HOME_SALE_SIGHTSEEING_STATUS.forEach((ele) => {
    ...
    data?.list.forEach((ele) => {
      const rowHtml = `
         <tr class="text-center">
            ...
            <td><b class="txt-status-${String(ele.status).toLocaleLowerCase()}">${VARIABLE_ENUM.HOME_SALE_SIGHTSEEING_STATUS.find((fi) => fi.value == ele.status)?.text ?? ''}</b></td>
