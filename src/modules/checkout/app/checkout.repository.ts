...
import { CheckoutCurrentPackageAppResDto, CheckoutPackageByExpireDayAppResDto } from './checkout.response';
...
export class CheckoutAppRepository {
  ...
  async getCurrentPackage(userCode: string): Promise<CheckoutCurrentPackageAppResDto | null> {
    ...
    return rows.length ? (rows[0] as CheckoutCurrentPackageAppResDto) : null;
  }

  async getPackageByExpireDay(expireDay: number): Promise<CheckoutPackageByExpireDayAppResDto | null> {
    ...
    return rows.length ? (rows[0] as CheckoutPackageByExpireDayAppResDto) : null;
  }
