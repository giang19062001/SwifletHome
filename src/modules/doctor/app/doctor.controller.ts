...
export class DoctorAppController {
  ...
  async requestDoctor(@GetUserApp() user: TokenUserAppResDto, @Body() dto: CreateDoctorDto) {
  ...
  @ApiOkResponse({ type: NumberOkResponseDto })
  async deleteRequestFile(@Param('seq') seq: number, @GetUserApp() user: TokenUserAppResDto): Promise<number> {
